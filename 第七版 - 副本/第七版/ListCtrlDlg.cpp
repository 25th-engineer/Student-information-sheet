...
void CListCtrlDlg::OnButtonOpen()
{
	...
	/*CString *strstr = new CString[nCol];
	for (int j = 0; j < nCol;j++)
	{
		strstr[j] = m_List.GetItemText(0, j);//获取列表控件中某行某列的数据
	}
	m_List.DeleteItem(0);
	nItems = m_List.GetItemCount();  //获取现在的行数

	m_List.InsertItem(nItems, strstr[0]);  //插入行
	for (int i = 1; i<nCol; i++)
	{
		m_List.SetItemText(nItems, i, strstr[i]);    //设置数据
	}*/
	...
	//二维数组初始化
	nItems = m_List.GetItemCount();  //获取现在的行数
	nCol = m_List.GetHeaderCtrl()->GetItemCount(); //获取现在的列数
	CString **m_data;
	int index;
	m_data = new CString*[nItems];//生成一个指针指针的一维数组==二维数组
	for (index = 0; index < nItems; index++)
	{
		m_data[index] = new CString[nCol];//为二维数组分配内存
	}
	for (int i = 0; i < nItems;i++)
		for (int j = 0; j < nCol; j++)
		{
			m_data[i][j] = m_List.GetItemText(i, j);//获取列表控件中某行某列的数据
		}
	...
	{	
		...
		{
			if (_ttoi(m_data[i][0]) >= _ttoi(m_data[j][0]))
			{
	...
	for (int i = 0; i < nItems; i++)
	{
		m_List.InsertItem(i, m_data[i][0]);
		for (int j = 1; j < nCol; j++)
		{
			m_List.SetItemText(i, j, m_data[i][j]);
		}
	}
}
...
//	CString str;
	...
	for (int j = 1; j <= nCol; j++)
	{
		...
		range.AttachDispatch(range.get_Item(COleVariant((long)1), COleVariant((long)j)).pdispVal);
		rValue = COleVariant(range.get_Value2());
		rValue.ChangeType(VT_BSTR);
		//if (CString(rValue.bstrVal) != "")
		//{
			strstr[j-1] = CString(rValue.bstrVal);
		//}
		//else
		//{
		//	continue;
	...
	for (int j = 0; j < nCol+1;j++)
	{
		if (j==0)
		{
			m_List.InsertColumn(j, _T("计数"), LVCFMT_CENTER, 180);
		}
		else
		{
			m_List.InsertColumn(j, strstr[j-1], LVCFMT_CENTER, 180);
		}
	...
	for (int i = 2; i <= 200; i++)
	{
		...
		//{
			for (int j = 0; j <nCol+1; j++)
			{
					...
					{
						CString strto;
						strto.Format(_T("%d"), m_count+1);
						m_List.InsertItem(m_count, strto);
					}
					else
					{
						m_List.SetItemText(m_count, j, strstr[j-1]);
					}
			}	
		//}
			for (int j = 0; j < nCol; j++)
	...
	int nCount = m_List.GetItemCount();//获取列表控件行数
	for (int k = nCount; k > 0; k--)
	{
		if (m_List.GetItemText(k, 1) == "")
		{
			m_List.DeleteItem(k);
		}
	}
...
// 获取ODBC中Excel驱动
CString CListCtrlDlg::GetExcelDriver()
{
	char szBuf[2001];
	WORD cbBufMax = 2000;
	WORD cbBufOut;
